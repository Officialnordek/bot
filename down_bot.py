...
debug = True
defaultKey = "5815914877:AAGDcLaeWeerf8Lbv8vUtBzAmuKwKktHiQg"
# private.BOT_KEY
defaultID = "-735245399" # 
...
class DownBot:
    def __init__(self):
        ...
        self.errors = {}
        self.errors['rpc'] = {}
        self.errors['rpc']['https://mainnet-rpc.nordekscan.com'] = 0
        self.errors['rpc']['https://mainnet-rpc.nordekscan.com'] = 0
        self.errors['rpc']['https://mainnet-rpc.nordekscan.com'] = 0
        self.errors['rpc']['https://mainnet-rpc.nordekscan.com'] = 0
    ...
    def fillSupplies(self):
        ...
        print(" self.stats['circSupply']",  self.stats['circSupply'])
        self.numberOfDelegates = getTotalDelegates()
        #Division by zero  fix luan
        self.APY = self.stats['circSupply']['staked'] and ((43150)/self.stats['circSupply']['staked']) * 100 * 365 * 0.85 or 0
    ...
    def parseNodes(self):
        if os.path.exists(os.path.join(__location__, nodeList)):
            ...
            self.nodes = {}
        print('~~~~~~~~~~~~~  self.saveSettings self.nodes', self.nodes)
    ...
    def grabAdmins(self, bot):
        ...
        printLogs(self.settings["ChatID"])
        ...
        if (user_id in self.settings["admins"]):
            if command == '/add_node':
                ...
                for node in listOfNodes:
                    print('nodes', node)
                    node = Web3.toChecksumAddress(node.strip())
                    print('nodes toChecksumAddress', node)
                    if node in self.nodes:
    ...
    def checkBalance(self):
        ...
        test = 1
        for node in self.nodes:
            if(str(node) == '0x878e1d7bd9e68830d1eac8f708898c0778111a5e'):
        ...
        #
                ...
                balance = getBalance('0xd095a3145767d72fb9f75cfe95bb37959b8c4a5a')
                message = ''
                if balance['eth'] <= self.settings["EthWarning"]:
                    message += 'Eth balance low (' + str(balance['eth']) + ')\n'
        #
                if message != '':
                    ...
                    message = user + ' ' + str('0xd095a3145767d72fb9f75cfe95bb37959b8c4a5a') + '\n' + message
                    #self.bot.send_message(self.settings["ChatID"], message)
