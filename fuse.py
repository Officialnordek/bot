...
INFURA_KEY = "https://mainnet.infura.io/v3/59de75d669054649ac47d608d2ccec47"
def getValidators():
    web3Fuse = Web3(Web3.HTTPProvider(contractABI.RPC_ADDRESS))
    print(contractABI.RPC_ADDRESS)
...
def getBalance(node):
    DECIMAL = 10 ** 18
    web3Eth = Web3(Web3.HTTPProvider(INFURA_KEY))
    web3Fuse = Web3(Web3.HTTPProvider(contractABI.RPC_ADDRESS))
...
def getValStats():
    ...
    for val in vals:
        ...
        print("valFee " + str(fuseConsensusContract.functions.validatorFee(val).call() ))

        returnDict[Web3.toChecksumAddress(val)]['valFee'] = (fuseConsensusContract.functions.validatorFee(val).call() / DECIMAL) * 100
        returnDict[Web3.toChecksumAddress(val)]['ratioOfStake'] =totalStaked and  (returnDict[Web3.toChecksumAddress(val)]['totalStaked'] / totalStaked) * 100 or 0
...
def getTotalSupply(block):
    initSupply = 2100000000
    blocksPerYear = 6307200
    
    burnt = 8173860.51897
    rewardsADDR = "0x7429fB101C317743369f3A161A6Ee78c3Ac45ADd"
    
    web3Fuse = Web3(Web3.HTTPProvider(contractABI.RPC_ADDRESS))
    
    rewardContract = create_contract(web3Fuse, contractABI.REWARDS_CONTRACT, Web3.toChecksumAddress(str(rewardsADDR)))

    rewards = list(fetch_events(rewardContract.events.RewardedOnCycle,from_block=1))
    total = 0
...
def getCircSupply(totalSupply, block, lockedAccountsList):
    ...
    # CONTRACT_ADDRESS = "0x8335C47E56CF0798d34c14550D9622E841851851"
    WFUSE_CONTRACT = "0xe1144e2afeBbf7d86253cd6A82C98683FBAcf4c6"
    wfuseWallet = Web3.toChecksumAddress("0x878e1d7bd9e68830d1eac8f708898c0778111a5e")
...
def grabDataFromGraphQL():
    ...
    url = "https://thegraph.nordekscan.com/subgraphs/name/nrkchain/fuse-consensus"
...
def checkIfRelayed(address):
    ...
    for trans in transactions:
        if Web3.toChecksumAddress(trans['to']) == Web3.toChecksumAddress('0x8db1436097E3d16DD366c138F6953Fb7ee9a96Ad'):
...
def getOpenBallots():
    ...
    if len(activeBallots) != 0:
        ...
        for ballot in activeBallots:
            returnData[ballot] = {}
            ballotInfo = fuseVotingContract.functions.getBallotInfo(ballot,Web3.toChecksumAddress('0x8db1436097E3d16DD366c138F6953Fb7ee9a96Ad')).call()
